...
#if ILITEK_PLAT != ILITEK_PLAT_MTK && !defined(ADV_DEBIAN)
#if defined(CONFIG_FB) || defined(CONFIG_DRM)
static int ilitek_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data) {
#if defined(CONFIG_DRM) && (ILITEK_PLAT == ILITEK_PLAT_QCOM) 
...
static int ilitek_get_gpio_num(void)
{
	...
	ilitek_data->power_gpio = of_get_named_gpio(np, "ilitek,power-gpio", 0);
	if (ilitek_data->power_gpio < 0)
		tp_log_err("power_gpio = %d\n", ilitek_data->power_gpio);
...
#endif
	tp_log_info("reset_gpio = %d irq_gpio = %d power_gpio = %d\n", ilitek_data->reset_gpio, ilitek_data->irq_gpio, ilitek_data->power_gpio);
	return ret;
}
...
static int ilitek_request_gpio(void)
{
	...
	if (ilitek_data->power_gpio > 0) {
		ret = gpio_request(ilitek_data->power_gpio, "ilitek-power-gpio");
		if (ret) {
			tp_log_err("Failed to request power_gpio so free retry\n");
			gpio_free(ilitek_data->power_gpio);
			ret = gpio_request(ilitek_data->power_gpio, "ilitek-power-gpio");
			if (ret)
				tp_log_err("Failed to request power_gpio\n");
		}
		if (ret) {
			tp_log_err("Failed to request power_gpio\n");
		} else {
			ret = gpio_direction_output(ilitek_data->power_gpio, 1);
			if (ret)
				tp_log_err("Failed to direction output rest gpio err\n");
		}
	}
...
#elif ILITEK_PLAT == ILITEK_PLAT_ROCKCHIP
	gpio_direction_output(ilitek_data->power_gpio, 1);
#else
