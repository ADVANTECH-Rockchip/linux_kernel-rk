...
/**************************************************************/
int hy46xx_write_reg(struct i2c_client * client, uint8_t regaddr, uint8_t regvalue)
{
...
/************************************************************/
int hy46xx_read_reg(struct i2c_client * client, uint8_t regaddr, uint8_t * regvalue)
{
...
#endif
/************************************************************/
void hy46xx_reset_tp(struct hy46xx_ts_data *hy46xx_ts)
{
    gpiod_direction_output(hy46xx_ts->pwen_gpio, 0);	
    msleep(10);
    gpiod_direction_output(hy46xx_ts->pwen_gpio, 1);
    msleep(10);
    gpiod_direction_output(hy46xx_ts->reset_gpio, 1);	
    msleep(10);
    gpiod_direction_output(hy46xx_ts->reset_gpio, 0);
    msleep(10);
}
                                    ...
                                    const char *buf, size_t count)
{
    ...
    int retval;
    uint16_t wmreg=0;
    uint8_t regaddr=0xff,regvalue=0xff;
    uint8_t valbuf[5]= {0};
...
static int hy46xx_init_gpio(struct device *dev,
				struct hy46xx_ts_data *data)
{
	#if 0
	...
	pdata->reset_gpio = of_get_named_gpio(np,"hy46xx,reset_gpio", 0, &pdata->reset_gpio);
	pr_info("%s(), reset_gpio: %d \n", __func__, pdata->reset_gpio);
	
	pdata->en_gpio = of_get_named_gpio(np,"hy46xx,en_gpio", 0, &pdata->en_gpio);
	pr_info("%s(), en_gpio: %d \n", __func__, pdata->en_gpio);
	
	pdata->irq_gpio = of_find_property(np,"interrupts", NULL);
	pr_info("%s(), irq_gpio: %d \n", __func__, pdata->irq_gpio);
	#else
	
	data->int_gpio = devm_gpiod_get_optional(dev, "int", GPIOD_IN);
	if (IS_ERR_OR_NULL(data->int_gpio)){
		printk("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXxxx\n");
		return PTR_ERR(data->int_gpio);
	}
	else{
		printk("* int_gpio: %s %s\n",data->int_gpio->label,data->int_gpio->name);
	}
	
	/* reset */
	data->reset_gpio = devm_gpiod_get_optional(dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR_OR_NULL(data->reset_gpio)){
		dev_warn(dev,"Could not get named GPIO for reset!\n");
	}
	else{
		printk("* reset_gpio: %s %s\n",data->reset_gpio->label,data->reset_gpio->name);
		gpiod_direction_output(data->reset_gpio, 0);
	}

	/* power */
	data->pwen_gpio = devm_gpiod_get_optional(dev, "pwen", GPIOD_OUT_HIGH);
	if (IS_ERR_OR_NULL(data->pwen_gpio)){
		dev_warn(dev, "Could not get named GPIO for power enable!\n");
	}
	else{
		printk("* pwen_gpio: %s %s\n",data->pwen_gpio->label,data->pwen_gpio->name);
		gpiod_direction_output(data->pwen_gpio, 1);
		while(0)
		msleep(500);
	}
...
static irqreturn_t hy46xx_ts_interrupt(int irq, void *dev_id)
{
    ...
    //int ret = 0;
    disable_irq_nosync(hy46xx_ts->gpio_int_irq);
    hy46xx_read_Touchdata(hy46xx_ts);
    hy46xx_report_value(hy46xx_ts);
    enable_irq(hy46xx_ts->gpio_int_irq);
    return IRQ_HANDLED;
}
/**********************************************************************************/
static int hy46xx_ts_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
    ...
    struct hy46xx_platform_data *pdata = NULL;
    struct hy46xx_ts_data *hy46xx_ts;
    unsigned long irqflags; //ted
    struct input_dev *input_dev;
    int err = 0;
    unsigned char IC_FW_Verson;
    //unsigned char I_File_Version;  //gcc say  unused variable
    unsigned char uc_reg_addr;
    printk("[hycon] %s,%d -->\n",__func__,__LINE__);
...
/////
    /*
    the pdata come from board-xxx.c originally,
    since now I change it to device tree,
    we need to allocate memory here and parse device tree
    */
    pdata = devm_kzalloc(&client->dev,
                         sizeof(struct hy46xx_platform_data),
                         GFP_KERNEL);
    ...
    /*
     * should parse dt for device properties, but the original driver for k2.6 do not have dt node,
     * keep in mind to add dt node later
     */

	err = hy46xx_init_gpio(&client->dev, hy46xx_ts);
	if (err) 
		return err;
...
/////
    ...
    //hy46xx_ts->gpio_int_irq = 14;	 //WTF, that is for s3c24xx platform!!!
    ...
    hy46xx_ts->pdata->irq = hy46xx_ts->gpio_int_irq;
     ...
     */
    client->irq = gpiod_to_irq(hy46xx_ts->int_gpio);		/*should get irq pin from dt,GPIO3_B6, ted*/
    irqflags = client->dev.of_node ? 0 : IRQF_TRIGGER_FALLING;

    err = devm_request_threaded_irq(&client->dev, client->irq,
                                    NULL, hy46xx_ts_interrupt,
                                    irqflags | IRQF_ONESHOT,
                                    client->name, hy46xx_ts);
    if (err < 0)
    {
        dev_err(&client->dev, "hy46xx_probe: request irq failed\n");
        goto exit_irq_request_failed;
    ...
    input_mt_init_slots(input_dev, CFG_MAX_TOUCH_POINTS , 0);  //remember to check the 3rd argumemt value, 0 for temporary to avoid compile error
...
module_i2c_driver(hy46xx_ts_driver);
#if 0 //unused
/************************************************************/
